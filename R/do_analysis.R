...
fig_scatter_dd_n_i <- ggplot(smp_da, aes(x = n_i, y = dd_da)) +
  geom_bin2d(
    aes(fill = stat(log(count))),  bins = c(100, 100)
  ) +
  labs(x = "Net Income", y = "Dechow and Dichev DA") +
  scale_fill_trr266_c() + 
  theme_trr(axis_y_horizontal = FALSE)

fig_scatter_dd_r_d <- ggplot(smp_da, aes(x = r_d, y = dd_da)) +
  geom_bin2d(
    aes(fill = stat(log(count))),  bins = c(100, 100)
  ) +
  labs(x = "RD Expense", y = "Dechow and Dichev DA") +
  scale_fill_trr266_c() + 
  theme_trr(axis_y_horizontal = FALSE)
